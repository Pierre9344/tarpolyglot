...
#' @param script Path to the Python script to run (required). Accepts a file path or an inline [tar_code()] carrier; see the "Script arguments" section below.
#' @param pre_script Optional path to an R script run before the Python script. It is evaluated in the step environment, which already holds the named `inputs`. To hand objects to Python, assign a named list `to_py` in this script; each element is pushed as a top-level variable in the Python `__main__` module. Accepts a file path or an inline [tar_code()] carrier; see the "Script arguments" section below.
#' @param post_script Optional path to an R script run after the Python script. It is evaluated in the same environment, which now also holds `py` (the reticulate `__main__` module proxy) and `py_get(name)`. In `output = "object"` mode the value of its last expression becomes the target value; in `output = "file"` mode it must return a character vector of file paths. Accepts a file path or an inline [tar_code()] carrier; see the "Script arguments" section below.
...
#' @inheritParams tarpolyglot-shared-params
#' @inheritSection tarpolyglot-shared-params Script arguments
#' @param script Path to the Julia script to run (required). Accepts a file path or an inline [tar_code()] carrier; see the "Script arguments" section below.
#' @param pre_script Optional path to an R script run before the Julia script. It is evaluated in the step environment, which already holds the named `inputs`. To hand objects to Julia, assign a named list `to_jl` in this script; each element is `julia_assign()`ed as a variable in Julia's `Main` module. Accepts a file path or an inline [tar_code()] carrier; see the "Script arguments" section below.
