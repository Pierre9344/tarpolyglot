...
# the plain `targets` pattern. On the Rust constructor it additionally asks the
# crate to be compiled once and reused across every branch (see
# `tar_target_rs()`), instead of recompiling in each branch.
...
#'
#' A drop-in replacement for the \pkg{targets} [targets::tar_target()] pattern helper `map()`, for use unquoted in the `pattern` argument of a tarpolyglot constructor ([tar_target_rs()], [tar_target_py()], [tar_target_jl()], and their `_raw` forms).
#'
...
#' @seealso [tar_target_rs()], [tar_target_py()], [tar_target_jl()]
#' @examples
#' \dontrun{
#' list(
...
.tp_pattern_marker <- function(fn) {
  stop(
    ...
    "of a tarpolyglot constructor (tar_target_rs() / tar_target_py() / ",
    "tar_target_jl()); it is not a function to call directly.",
    call. = FALSE
  )
}
