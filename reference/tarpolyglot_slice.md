...
pattern helper `slice()`, for use unquoted in the `pattern` argument of
...
target and reuses it across the selected branches; on
...
- ...:
  ...
  targets `slice()` pattern.

- index:

  Integer vector of branch indices to keep, as in the targets `slice()`
  pattern.
...
#> [[1]]
#> <tar_stem> 
#>   name: rs_some_rust_lib 
#>   description: Compile the Rust crate once for rs_some 
#>   command:
...
#>   description:  
#>   command:
#>     tarpolyglot::run_rs_step_prebuilt(lib = rs_some_rust_lib, 
#>         post_script = "post.R", inputs = list(x = vals), output = "object", 
#>         files = NULL) 
#>   pattern:
#>     slice(vals, index = c(1, 3)) 
#>   format: rds 
#>   repository: local 
#>   iteration method: vector 
#>   error mode: stop 
