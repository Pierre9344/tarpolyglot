...
Build a targets step that runs Python, Julia, Rust, or C++. The
non-`_raw` forms use non-standard evaluation on `name`/`pattern` for
direct use in `_targets.R`; the `_raw` forms take a string `name` for
use inside targets factories.
...
while compiling Rust or C++ once, and a crew controller preconfigured
for per-step interpreter isolation.
...
compile once per pattern and reuse the result across branches: the Rust
crate in a `<name>_rust_lib` target, the C++ library in a
`<name>_cpp_lib` target. On the Python/Julia constructors they fall back
to the plain `targets` pattern, since a live interpreter has nothing to
compile.
...
- [`compile_cpp_lib()`](https://pierre9344.github.io/tarpolyglot/reference/compile_cpp_lib.md)
  : Compile a C++ step once for reuse across branches (worker behind
  tarpolyglot_map)
- [`run_cpp_step_prebuilt()`](https://pierre9344.github.io/tarpolyglot/reference/run_cpp_step_prebuilt.md)
  : Run a C++ step from a pre-compiled library (worker behind
  tarpolyglot_map)
