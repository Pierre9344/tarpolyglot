...
# for the no-TLS default polyglot_controller() uses. Where nanonext was built
# without TLS support that self-test fails with "Not supported" and no
# controller can be constructed. That is a property of the nanonext
# installation rather than of tarpolyglot, and nothing in this package can work
# around it, so the tests that need a real controller skip there instead of
# failing the check.
skip_if_no_crew_tls <- function() {
  testthat::skip_if_not(
    .tp_crew_tls_available(),
    "nanonext was built without TLS support, so crew cannot build a controller"
  )
}
...
test_that("polyglot_controller reports an unusable crew TLS layer", {
  ...
  # Simulate the platform CRAN saw: nanonext built without TLS support, so
